...
Assistant.prototype.add = function(data, callback) {
    ...
    this._.getAccessToken(function(err, token) {
        if (err)
            return callback(err);
        utils.post(config.assistant.add.format(token), data, callback); // 发送请求
    });
};
Assistant.prototype.update = function(data, callback) {
    ...
    this._.getAccessToken(function(err, token) {
        if (err)
            return callback(err);
        utils.post(config.assistant.update.format(token), data, callback); // 发送请求
    });
};
Assistant.prototype.send = function(data, callback) {
    ...
    this._.getAccessToken(function(err, token) {
        if (err)
            return callback(err);
        utils.post(config.assistant.send.format(token), data, function(err, res) {
            if (err)
                return callback(err);
            var _len = 0,
                _buf = [];
